import { siteForUrl } from '@/shared/sites';
import type {
  ContentMessage,
...
// Resolved from the shared registry — holds the player/video selectors for
// whichever platform we're on.
const SITE = siteForUrl(location.href);

let uaCues: Cue[] | null = null; // parsed translated cues
let uaSRT: string | null = null; // raw translated SRT (sent to the bridge for voice-over)
let overlayEl: HTMLDivElement | null = null;
let lastWindowSig: string | null = null; // signature of the last rendered prev|cur|next window
let settings: Settings = { ...DEFAULT_SETTINGS };

/** First element matching any selector in the list, or null. */
function firstMatch<T extends Element = HTMLElement>(selectors: string[] | undefined): T | null {
 ...
 */
function findCueWindow(cues: Cue[], t: number): CueWindow {
  let lo = 0;
...
function ensureOverlay(): HTMLDivElement | null {
  ...
  const container =
    ...
    document.querySelector<HTMLElement>('.plyr') ||
    document.querySelector<HTMLElement>('.jwplayer') ||
    document.querySelector<HTMLElement>('.video-js');
  ...
  for (const level of ['prev', 'current', 'next'] as const) {
...
function setOverlayText(text: string, mode: 'loading' | 'error' = 'loading'): void {
  const el = ensureOverlay();
  if (!el) return;
  el.dataset.mode = mode;
  const textEl = el.querySelector<HTMLElement>('.ua-subs-text');
  if (textEl) textEl.textContent = text;
}

/** Voice-over status badge — '' clears it. Independent of the subtitle stack. */
function setDubBadge(text: string): void {
  const el = ensureOverlay();
  if (!el) return;
  const badge = el.querySelector<HTMLElement>('.ua-subs-dub');
  if (badge) badge.textContent = text || '';
}

/** 3-level subtitle display — shows the stack and hides the status line. */
function setSubtitleLevels(prev?: string, cur?: string, next?: string): void {
  const el = ensureOverlay();
  if (!el) return;
  ...
  set('.ua-subs-prev', prev);
  set('.ua-subs-current', cur);
  set('.ua-subs-next', next);
}
 ...
 */
function suppressNativeCaptions(): void {
...
function attachToVideo(): void {
  const video =
    firstMatch<HTMLVideoElement>(SITE?.video) || document.querySelector<HTMLVideoElement>('video');
  ...
  video.addEventListener('timeupdate', () => {
    ...
    lastWindowSig = sig;
    setSubtitleLevels(uaCues[w.prev]?.text, uaCues[w.cur]?.text, uaCues[w.next]?.text);
  ...
  video.addEventListener('pause', () => {
    if (dubAudio && !dubAudio.ended) dubAudio.pause();
  });
  video.addEventListener('play', () => {
    if (dubAudio && dubAudio.src && !dubAudio.ended && dubAudio.paused) {
      dubAudio.play().catch(() => {});
    }
  });
...
/** Vendor-prefixed pitch flags aren't in lib.dom. */
interface DubAudioElement extends HTMLAudioElement {
  mozPreservesPitch?: boolean;
  webkitPreservesPitch?: boolean;
}

const dubUrls = new Map<string, string>(); // cueId → blob URL
let dubAudio: DubAudioElement | null = null; // single reused <audio>
let dubVideo: HTMLVideoElement | null = null; // the <video> we duck
let spokenId: string | null = null; // cue currently voiced — guards against re-triggering
let origVolume: number | null = null; // saved video volume, restored after ducking

function getDubAudio(): DubAudioElement {
  if (dubAudio) return dubAudio;
  const audio = new Audio() as DubAudioElement;
 ...
 */
function computeRate(cue: Cue, duration: number): number {
  ...
  const rate = Number(settings.dubSpeed);
  return isFinite(rate) && rate > 0 ? Math.min(3, Math.max(0.5, rate)) : 1;
}
 ...
 */
function clearDub(): void {
  try {
    send({ type: 'CANCEL_TTS' });
  } catch {
...
function maybeSpeak(cueIdx: number, t: number): void {
  ...
  audio.playbackRate = computeRate(cue, NaN);
  audio.onloadedmetadata = () => {
    audio.playbackRate = computeRate(cue, audio.duration);
  };
  duck();
  audio
    .play()
...
chrome.runtime.onMessage.addListener((msg: ContentMessage) => {
  switch (msg.type) {
    ...
    case 'SETTINGS': {
      ...
      if (msg.dubSpeed !== undefined) {
        settings.dubSpeed = msg.dubSpeed;
        applyDubSpeed();
      }
      applySettings();
      ...
      else if (dubChanged && !settings.dub) {
        clearDub();
        setDubBadge('');
      }
      break;
...
// Ask background for current translation state (handles the race where the
// track was requested before this content script was ready to receive messages).
chrome.runtime.sendMessage(
  { type: 'CONTENT_READY' } satisfies RequestMessage,
  (resp?: ContentReadyResponse) => {
    console.log(
      '[ua-subs] Background state:',
      chrome.runtime.lastError?.message || resp?.status || 'no response'
    );
